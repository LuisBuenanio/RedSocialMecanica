...
function Chat({ route }) {
    ...
    const onSend = useCallback((m = []) => {
        const messagesWillSend = [{ ...m[0], sent: true, received: false,  file: m[0].file ? [{ name: m[0].text, url: m[0].file }] : [], // Añade información sobre archivos adjuntos
    }];
    ...
    const pickFile = async () => {
        try {
            ...
            if (result.type === 'success') {
                ...
                await uploadFileAsync(uri, name);
            } else if (result.type === 'cancel') {
                console.log('Se canceló la selección de archivo');
            } else {
    ...
    async function uploadFileAsync(uri, name) {
        try {
            ...
            onSend([{
                _id: randomString,
                createdAt: new Date(),
                text: name, // Mostrar el nombre del archivo en el chat
                file: uploadedFileString, // Guardar la URL del archivo subido
                user: {
    ...
    function renderSend(props) {
        return (
            <>
                ...
                <TouchableOpacity style={styles.addImageIcon} onPress={pickFile}>
                    <View>
                        <Ionicons
                            name='attach-outline'
                            size={32}
